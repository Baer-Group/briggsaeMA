...
#SBATCH --job-name=10xcov # Job name
...
#SBATCH --output=10xcov_%j.out # Standard output and error log
...
out="common_10x_summary.txt"

# Header

echo -e "Sample1\tSample2\tCommonSites_10x" > "$out"
...
while IFS=$'\t' read -r bam1 bam2; do
  ...
  shared=$(samtools depth -a "$bam1" "$bam2" | awk '$3>=10 && $4>=10' | wc -l)
