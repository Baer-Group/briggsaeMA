...
# Extended version: analyze THREE context orientations
#
...
#
# This script is intentionally modified only enough to support all three
# context types. The original 96-channel pyrimidine-collapsed analysis is
# kept for the CENTERED context only, because it is naturally defined when
# the mutable site is the middle base. The 64-motif exact-context analysis
# is run for all three context orientations.
#
# Required inputs:
...
#
# Optional / upstream:
#   make_snp_bed_three_contexts.R to generate the three BED files
# =============================================================================
...
parse_fasta_context <- function(fasta_file, focal_idx) {
  ...
  fasta_df <- data.frame(
    ...
    mutate(
      ...
      focal_matches_ref = focal_base == toupper(REF)
    )

  n_bad <- sum(!fasta_df$focal_matches_ref)
  cat(sprintf("  Parsed %d usable 3-bp contexts; focal base != REF: %d\n",
              nrow(fasta_df), n_bad))
  fasta_df
}

compute_64_rates <- function(muts_base, fasta_df, tri_genome_64, focal_idx, context_id, context_label, focal_name) {
  ...
  line_strain <- muts_local %>% distinct(Sample, Strain)
...
# =============================================================================
# STEP 2: GENOME TRINUCLEOTIDE COUNTS
# =============================================================================
...
# =============================================================================
# STEP 3: ORIGINAL 96-CHANNEL ANALYSIS FOR CENTERED CONTEXT ONLY
# =============================================================================
cat("\n=== Centered 96-channel analysis (kept from original script) ===\n")
...
muts_center <- muts_base %>%
  left_join(center_fasta %>% select(CHROM, POS, REF, ALT, context),
            by = c("CHROM", "POS", "REF", "ALT"))
...
line_strain    <- muts_96 %>% distinct(Sample, Strain)
...
# =============================================================================
# STEP 4: 64-MOTIF ANALYSIS FOR ALL THREE CONTEXT TYPES
# =============================================================================
...
for (i in seq_len(nrow(context_configs))) {
  ...
  validation_rows[[i]] <- data.frame(
    ...
    n_records  = nrow(fasta_df),
    focal_base_not_ref = sum(!fasta_df$focal_matches_ref),
    stringsAsFactors = FALSE
  )
  ...
  write.csv(mut_counts_64,
            paste0("trinucleotide_64motif_rates_", cfg$outfile_stub[[1]], ".csv"),
            row.names = FALSE)
  cat(sprintf("Saved: trinucleotide_64motif_rates_%s.csv\n", cfg$outfile_stub[[1]]))
  ...
  save_fig(fig_64_combined,
           paste0("Fig_trinucleotide_64motif_combined_", cfg$outfile_stub[[1]]),
           16, 9.5)

  save_fig(p64_hk,
           paste0("Fig_trinucleotide_64motif_HK104_", cfg$outfile_stub[[1]]),
           16, 4.75)

  save_fig(p64_pb,
           paste0("Fig_trinucleotide_64motif_PB800_", cfg$outfile_stub[[1]]),
           16, 4.75)
}

validation_df <- bind_rows(validation_rows)
write.csv(validation_df, "trinucleotide_64motif_context_validation.csv", row.names = FALSE)
cat("Saved: trinucleotide_64motif_context_validation.csv\n")

all_64_df <- bind_rows(all_64)
write.csv(all_64_df, "trinucleotide_64motif_rates_all_contexts.csv", row.names = FALSE)
cat("Saved: trinucleotide_64motif_rates_all_contexts.csv\n")

cat("\nDone. The script now supports all three context types with minimal changes.\n")
cat("Notes:\n")
cat("  - 96-channel / substitution-specific analysis is kept only for centered xYz.\n")
cat("  - 64-motif exact-context analysis is produced for centered, 3-prime, and 5-prime contexts.\n")
cat("  - This will generate many plots, but that is fine.\n")
