...
#   master_table.csv            (per-line table; supplies SR_callable_3x used
#                                for line-specific callable normalization)
#
# CHANGE LOG (v5):
#   * Normalization now uses a LINE-SPECIFIC callable fraction
#     (cf_i = SR_callable_3x_i / GENOME_SIZE) instead of one global CALLABLE_FRAC.
#   * Combined two-strain figures share ONE "Mean mu" y-axis
#     (plot_layout(axis_titles = "collect"); needs patchwork >= 1.2.0).
# =============================================================================
...
CALLABLE_FRAC <- 0.91   # SUPERSEDED: replaced by line-specific SR_callable_3x/GENOME_SIZE (kept for reference)
GENERATIONS   <- 238
GC_FRAC       <- 0.37
AT_FRAC       <- 0.63
N_HK104       <- 32
...
# =============================================================================
# STEP 4b: LINE-SPECIFIC CALLABLE FRACTION  (replaces the global CALLABLE_FRAC)
# Each line i gets its own callable fraction cf_i = SR_callable_3x_i / GENOME_SIZE
# from master_table.csv. The callable count of a motif in line i is then
#   genome_count(motif) * cf_i
# (this keeps the same "callability is uniform across motifs" assumption the
#  global-fraction version used -- it just lets the total vary line by line).
# =============================================================================
cat("Loading line-specific callable fractions from master_table.csv...\n")

muts <- muts %>% mutate(Sample = as.character(Sample))

line_callable <- read.csv("master_table.csv", stringsAsFactors = FALSE) %>%
  transmute(Sample             = as.character(Line),
            line_callable_frac = SR_callable_3x / GENOME_SIZE)

cat(sprintf("  %d lines; callable fraction range %.3f - %.3f (mean %.3f)\n",
            nrow(line_callable),
            min(line_callable$line_callable_frac, na.rm = TRUE),
            max(line_callable$line_callable_frac, na.rm = TRUE),
            mean(line_callable$line_callable_frac, na.rm = TRUE)))
...
full_grid <- line_strain %>%
  ...
  left_join(line_callable, by = "Sample") %>%
  mutate(rate = n_mut / (callable_count * line_callable_frac * GENERATIONS))
...
fig_combined <- (p_hk / p_pb) +
  plot_layout(axis_titles = "collect")   # single shared "Mean mu" y-axis (patchwork >= 1.2.0)
# --- Fallback if patchwork < 1.2.0 (uncomment): ---
# fig_combined <- ((p_hk & theme(axis.title.y = element_blank())) /
#                  (p_pb & theme(axis.title.y = element_blank())))
# fig_combined <- wrap_elements(grid::textGrob(ylab_rate, rot = 90,
#                   gp = grid::gpar(fontsize = 22))) | fig_combined
# fig_combined <- fig_combined + plot_layout(widths = c(1, 40))
...
full_grid_64 <- line_strain %>%
  ...
  left_join(line_callable, by = "Sample") %>%
  mutate(rate_64 = n_mut_64 / (callable_count_64 * line_callable_frac * GENERATIONS))
...
fig_64_combined <- (p64_hk / p64_pb) +
  plot_layout(guides = "collect", axis_titles = "collect") &   # one shared y-axis + one legend
