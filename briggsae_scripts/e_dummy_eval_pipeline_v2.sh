#!/usr/bin/env bash
#SBATCH --job-name=b1_dummy_eval_v2
...
# =============================================================================
# C. briggsae Short-Read Dummy Reference — Variant Recall Evaluation Pipeline
# v2 -- GENOME-WIDE FtR (counts absent sites as universal recall failures)
# =============================================================================
# CHANGES FROM v1 (2026-05-24)
#   v1 underestimated FtR because:
#     (a) GATK SelectVariants used --exclude-non-variants true, which dropped
#         sites where every sample called 0/0 (a true universal FN — the kind
#         of site we MOST want to count).
#     (b) The Python evaluator then SKIPPED any ground truth site not found
#         in the (already filtered) biallelic VCF, treating it as
#         "unevaluable" rather than as a universal recall failure.
#   Net effect: p_ftr was effectively a within-recallable-site miss rate
#   (~5%), which is LESS than the genome's uncallable fraction (~9.1%) and
#   thus inconsistent with a genome-wide interpretation.
#
#   v2 changes:
#     1. Drop `--exclude-non-variants true` from SelectVariants. All-0/0
#        sites emitted by GenotypeGVCFs are now retained in the biallelic
#        VCF; per-sample 0/0 and ./. genotypes at those sites are counted
#        as FN and missing, respectively.
#     2. In the Python evaluator, ground truth sites not present in the
#        biallelic VCF (i.e., sites GenotypeGVCFs dropped because no sample
#        had any variant evidence) are counted as universal FtR -- they
#        contribute N_samples failure events each to the denominator. A
#        new 'absent' category is reported in the per-sample summary, and
#        the FTR_rate column now includes it.
#
# CAVEAT: A small minority of 'absent' sites may be multi-allelic spanned
# deletions dropped by `bcftools view --max-alleles 2`; these are over-
# counted as FtR. The bias is small for SNP dummies (1:1 coords) and modest
# for indel dummies.
#
# PURPOSE:
...
#       ./.     → missing/masked     (failure to recall, no coverage)
#       absent  → site not in pipeline VCF (universal failure, all samples)
#                  -- in v2 this is COUNTED AS FtR, no longer excluded
#
...
#   5. (v1: absent sites EXCLUDED from denominator)
#      v2: absent sites now COUNTED as universal FtR (one event per sample)
...
run_dummy_pipeline() {
    ...
    for dp in "${DP_LEVELS[@]}"; do
        ...
        gatk --java-options "$JAVA_MEM" SelectVariants \
            ...
            -O "$CLEAN"
        # v2: NO --exclude-non-variants true
        # Reason: that flag dropped sites where every sample called 0/0
        # (true universal FN events we want to count as FtR). Without it,
        # all-0/0 dummy sites that made it through GenotypeGVCFs are
        # retained in the biallelic VCF and per-sample 0/0 / ./. genotypes
        # are correctly classified by the Python evaluator below.
...
sample_counts = {s: {'1/1':0,'0/1':0,'0/0':0,'missing':0,'absent':0,'other':0,'total':0}
                 for s in samples}
gt_rows    = []
n_evaluable = 0
n_absent    = 0   # v2: was n_skipped; now counted as universal FtR
...
for (chrom, pos), (ref, alt, info) in gt_sites.items():
    ...
    # v2: sites absent from pipeline VCF are universal recall failures.
    # GenotypeGVCFs only emits sites where at least one sample has variant
    # evidence; if a dummy site is missing from the joint VCF (and thus from
    # the biallelic VCF) it means NO sample called the variant. Count this
    # as one FtR event per sample, in the 'absent' category.
    if (chrom, pos) not in vcf_calls:
        n_absent += 1
        for samp in samples:
            sample_counts[samp]['absent'] += 1
            sample_counts[samp]['total']  += 1
            gt_rows.append({
                'CHROM': chrom, 'POS': pos, 'REF': ref, 'ALT': alt,
                'INFO': info, 'INDEL_BIN': bin_label,
                'SAMPLE': samp, 'GT': 'absent', 'CATEGORY': 'absent'
            })
        continue
...
print(f"  Absent sites    : {n_absent} — counted as universal FtR "
      f"(N_samples = {len(samples)} events per absent site)")
...
with open(recall_out, 'w', newline='') as f:
    fieldnames = ['SAMPLE','Total','Recalled_1/1','Het_0/1','FN_0/0',
                  'Missing','Absent','Other','Recall_rate','FN_rate','FTR_rate']
    ...
    for samp in samples:
        ...
        absent   = c['absent']
        other    = c['other']
        ftr      = het + fn + miss + absent + other   # v2: includes absent
        w.writerow({
            'SAMPLE':       samp,
            'Total':        tot,
...
# ------------------------------------------------------------------
if dummy_type == 'indel':
    bin_counts = defaultdict(lambda: {'1/1':0,'0/1':0,'0/0':0,'missing':0,
                                       'absent':0,'other':0,'total':0})
    for row in gt_rows:
        b = row['INDEL_BIN']
    ...
    with open(len_out, 'w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=[
            ...
            'FN_0/0','Missing','Absent','Recall_rate','FN_rate','FTR_rate'],
            delimiter=',')
        w.writeheader()
        for b in bin_order:
...
print(f"  {'SAMPLE':<40}  {'Total':>6}  {'1/1':>6}  {'0/1':>6}  {'0/0':>6}  "
      f"{'Missing':>8}  {'Absent':>7}  {'Recall%':>8}  {'FN%':>6}  {'FTR%':>6}")
print("  " + "-"*110)
all_tot = all_rec = all_het = all_fn = all_miss = all_abs = 0
for samp in samples:
    c = sample_counts[samp]
    tot = c['total']
    if tot == 0: continue
